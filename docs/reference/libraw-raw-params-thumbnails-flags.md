...
`_RAWSPEED3`, `_RAWSPEED_BITS`. A binding should export these as booleans so callers can detect "lossy DNG
unsupported" (`!JPEG`) instead of guessing from an error.
...
`NO_BADPIXELMAP`, `BAD_DARKFRAME_FILE`, `BAD_DARKFRAME_DIM`, `NO_JASPER`, `RAWSPEED_PROBLEM`,
`RAWSPEED_UNSUPPORTED`, `RAWSPEED_PROCESSED`, `FALLBACK_TO_AHD`, `PARSEFUJI_PROCESSED`, `DNGSDK_PROCESSED`,
`DNG_IMAGES_REORDERED`, `DNG_STAGE2_APPLIED`, `DNG_STAGE3_APPLIED`, `RAWSPEED3_*`. Expose as a string array.

...
// T16: toSharp(sharpModule) attached to decode()'s and thumbnail()'s
// resolved results (identify() carries no pixel buffer, so it gets none) --
// see lib/tosharp.cjs's header comment for the raw-vs-JPEG dispatch.
...
const RESULT_DECORATORS = {
  decode: attachRawToSharp,
  thumbnail: (result) => attachThumbnailToSharp(result, 'format'),
};

...
class LibRawError extends Error {
  ...
  static fromNative(err, fallbackStage) {
    ...
    // `instanceof Error` fails across realms: the addon creates its errors
    // with the realm it was loaded in, while Jest's per-file vm contexts (and
    // any other vm-isolated consumer) have their own `Error`. util.types.
    // isNativeError() recognises a real Error from any realm, so cancellation
    // (`aborted`) and LibRaw codes are wrapped consistently everywhere.

...
// T16: toSharp(sharpModule) attached to imageSync()/image()'s and
// thumbSync()/thumb()'s resolved results -- see lib/tosharp.cjs's header
// comment for the raw-vs-JPEG dispatch. `type` is thumbSync()/thumb()'s
// format-string key (ImageFormatType -- distinct spelling from the fused
// thumbnail() helper's `format` key, see scripts/templates/index.d.ts.tpl).
const { attachRawToSharp, attachThumbnailToSharp } = require('./tosharp.cjs');

// Methods in METHODS/ASYNC_METHODS whose return value gets a toSharp()
// decorator, keyed by method name (present in either list; applied the same
// way regardless of sync/async since both hand back the plain result value,
// not a Promise, at the point this map is consulted -- see below).
const RESULT_DECORATORS = {
  imageSync: attachRawToSharp,
  image: attachRawToSharp,
  thumbSync: (result) => attachThumbnailToSharp(result, 'type'),
  thumb: (result) => attachThumbnailToSharp(result, 'type'),
};
...
const METHODS = [
  ...
  // src/generated/params.gen.cc). Synchronous, like the rest of this list --
  // validation is cheap and never touches LibRaw's decode path.
  'setParams',
  'setRawParams',
  'getParams',
  'getRawParams',
...
function wrapProcessor(NativeProcessor, LibRawError) {
  ...
  // T14a: `processor.metadata` is a getter (backed by the native
  // InstanceAccessor of the same name, src/processor.cc), not a method --
  // handled separately from METHODS/ASYNC_METHODS below, which only ever
  // define ordinary function properties.
  Object.defineProperty(Processor.prototype, 'metadata', {
    get() {
      try {
        return this._native.metadata;
      } catch (err) {
        throw LibRawError.fromNative(err, 'metadata');
      }
    },
    enumerable: true,
    configurable: true,
  ...
  for (const method of METHODS) {
    const decorate = RESULT_DECORATORS[method];
    Processor.prototype[method] = function (...args) {
      try {
        const result = this._native[method](...args);
        if (decorate) decorate(result);
        return result;
      } catch (err) {
